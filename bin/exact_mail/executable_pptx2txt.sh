...
#
# command line bash shell script to convert PowerPoint pptx
# format to text on Mac, Linux or Unix
...
nslides=$(unzip -l "$pptx_filename" ppt/slides/slide*.xml | \
    awk '{print $2}' | tail -1)
...
for idx in $(seq 1 "$nslides");
  do
    ...
    unzip -qc "$pptx_filename" ppt/slides/slide"${idx}".xml | \
        perl -e 'while(<>) {  if (@list = ($_ =~ m/\<a:t\>(.+?)\<\/a:t\>/g)) { print "$_\n" for @list } }'
done
